...
With weekend_bookings AS (SELECT hotel_name,
      							 EXTRACT(ISODOW FROM booking_date) AS booking_day
							FROM bookings
							WHERE EXTRACT(ISODOW FROM booking_date) >= 6
						 )
		
	SELECT hotel_name,
	       COUNT(booking_day) AS total_weekend_bookings
...
| hotel_name  | total_weekend_bookings |
|-------------|------------------------|
| Hotel A |	8 |
| Hotel D |	5 |
| Hotel C | 4 |
...
WITH pay_types AS (SELECT  hotel_name,
						   CASE WHEN payment_type = 'Credit' THEN 1 
								ELSE 0
								END AS credit,
	...
	SELECT hotel_name,
	       SUM(credit) AS credit_payment,
		   SUM(cash) AS cash_payment
		FROM pay_types
		GROUP BY hotel_name
		ORDER BY hotel_name;
...
```
 ...
 | hotel_name | credit_payment | cash_payment |
 |------------|----------------|--------------| 
| Hotel A |	5 |	5 |
 | Hotel B |	5 |	5 |
| Hotel C | 5 |	5 |
